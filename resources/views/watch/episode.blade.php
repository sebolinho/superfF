...
@section('content')
    ...
    <img 
        ...
        alt="Pôster do episódio {{$episode->episode_number}} da temporada {{$episode->season_number}} de {{$listing->title}}, assista online grátis em HD"
        loading="lazy"
        title="{{$listing->title}} - Episódio {{$episode->episode_number}} da Temporada {{$episode->season_number}}"
    >
</div>
    ...
    Assistir Episodio {{$episode->name}} 
    <span class="text-xl text-gray-300 ml-2 before:content-['#']">
        {{ __(':number Temporada',['number' => $episode->season_number]) }} {{ __(':number Episódio',['number' => $episode->episode_number]) }}
    </span> 
    Online Grátis em HD
</h1>
