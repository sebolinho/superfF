   ...
   public function store(Request $request)
{
    ...
    $this->validate($request, [
        ...
        'tmdb_id' => 'required|integer', // Removido 'unique' para permitir substituição
    ]);

    // Recupera o tmdb_id do request
    ...
    $model->arguments = $request->input('arguments');
    ...
    if ($request->has('season')) {
        foreach ($request->input('season') as $key) {
            if ($key['season_number']) {
                ...
                $season->tmdb_id = $key['tmdb_id'] ?? null;
                $model->seasons()->save($season);

                if (isset($key['episode']) && isset($key['tmdb_id'])) {
    ...
    if ($isUpdate) {
        $message = __(':title updated', ['title' => $request->input('title')]);
    } else {
        $message = __(':title created', ['title' => $request->input('title')]);
    }

    return redirect()->route('admin.tv.index', $model->id)->with('success', $message);
}
